...
# LHC_2016_25ns_beforeTS1]$ python 016a_heat_load_period.py tstart=08-05-2016,08:00 tend=20-08-2016,05:30 plotaverage=n plotall=y timein=datetime tagfills=n varlists=AVG_ARC mode=norm_to_intensity normlength=none hourtickspac=24 plotmodel=y zeroat=1.
import time, sys
import pylab as pl
...
for i_fill, filln in enumerate(fill_list):
    ...
    fill_dict.update(tm.parse_timber_file('bs_temperatures_data_csvs/tempwup_data_fill_%d.csv'%filln, verbose=False))
...
if normalization_to_length_of is None:
    ...
    ax2.set_ylabel('Heat load\n[W/m]')
ax2.legend(bbox_to_anchor=(1.0, 1.05),  loc='upper left', prop={'size':12}, ncol=2)#, frameon=False)
ax2.set_ylim(0, None)
ax2.grid('on')
...
fig.subplots_adjust(left=.12, right=.80, hspace=.28, top=.89)
